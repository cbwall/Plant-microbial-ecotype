...
pacman::p_load("car", "lme4", "lmerTest", "pbkrtest", "emmeans", "ggplot2", "tidyverse", "dplyr", "labdsv", "grid", "gridExtra", "patchwork", "plyr", "mctoolsr", "vegan", "car", "metacoder", "multcompView", "patchwork", "pairwiseAdonis", "labdsv", "eulerr", "ggthemes", "remotes", "mctoolsr", "brglm2")
```
...
#prop.biomass.Water<-ggplot(longer_data,aes(Water,response,fill=Water)) +
  ...
  #ylab("Proportion of total biomass (g/g)") +
  #scale_fill_manual(values=c("paleturquoise3", "steelblue2", "skyblue4"))+ 
  #theme(
    #axis.text = element_text(color='black',size=10),
    #axis.title = element_text(color='black',size=12),
    #axis.ticks = element_line(color='black'),
...
#prop.plots<- prop.biomass.Water + prop.biomass.Inoc
...
### Plant mortality
```{r mortality import and subset, results='hide',echo=FALSE}
...
anova(glm.mort.full, test="Chisq") # no effect, but a trend, p=0.07

em.full.mort <- emmeans(glm.mort.full, pairwise~ Inoculum)
multcomp::cld(em.full.mort, Letters=letters)
```

Make a plot of mortality data.
```{r mortality prop data, results='hide'}
### final mortality data here
mortality_prop_data<-mortality.df %>%
  group_by(Water, Inoculum) %>% 
...
### plot it
mortality<-ggplot(mortality_prop_data,aes(Inoculum,prop,fill=Inoculum, color=Water))+
...
mortality
ggsave("figures/mortality.pdf", height=6, width=6)
...
ASV.T1 = filter_data(ASV.data, 'TimePoint', keep_vals = 'T1')
...
pair.mod.sh.T1<-pairwise.adonis(otuTAB.transp.sh.T1,factors=mtapairwise.sh.T1$water_inoculum)
pair.mod.sh.T1
...
# water pairwise
pair.mod.water.ro.T1<-pairwise.adonis(otuTAB.transp.ro.T1,
                          factors=c(mta.ro.T1$Water))
pair.mod.water.ro.T1

# inoc pairwise
pair.mod.inoc.ro.T1<-pairwise.adonis(otuTAB.transp.ro.T1,
                          factors=c(mta.ro.T1$Inoculum))
pair.mod.inoc.ro.T1

# interaction pairwise
pair.mod.wat.inoc.ro.T1<-pairwise.adonis(otuTAB.transp.ro.T1,
                          factors=c(mtapairwise.ro.T1$water_inoculum))
pair.mod.wat.inoc.ro.T1
```
...
shoot.T0T1 = filter_data(inputshootsonly, 'TimePoint', filter_vals = 'Control') # 203 samples
```
...
#make the model
model1.sh.T0T1<-adonis2(otuTAB.transp.sh.T0T1~Water*TimePoint,data=mta.sh.T0T1,
                permutations=999,method="bray")
model1.sh.T0T1

model3.sh.T0T1<-adonis2(otuTAB.transp.sh.T0T1~Water*TimePoint,data=mta.sh.T0T1,
                permutations=999,method="jaccard")
model3.sh.T0T1

model2.sh.T0T1<-adonis2(otuTAB.transp.sh.T0T1~Inoculum*TimePoint,data=mta.sh.T0T1,
                permutations=999,method="bray")
model2.sh.T0T1

model4.sh.T0T1<-adonis2(otuTAB.transp.sh.T0T1~Inoculum*TimePoint,data=mta.sh.T0T1,
                permutations=999,method="jaccard")
model4.sh.T0T1
```
...
# make a rarefaction curve of all T1 data
otuTAB.T0.shoot <- as.data.frame (T0.shoot$data_loaded)
...
inocNMDSmovement = filter_data(inputshootsonly, 'TimePoint', filter_vals = 'Control')
inoc1NMDSmovement =filter_data(inocNMDSmovement, 'Inoculum', keep_vals = '1')
inoc4NMDSmovement =filter_data(inocNMDSmovement, 'Inoculum', keep_vals = '4')
inoc5NMDSmovement =filter_data(inocNMDSmovement, 'Inoculum', keep_vals = '5')
inoc6NMDSmovement =filter_data(inocNMDSmovement, 'Inoculum', keep_vals = '6')
```
...
inocalone
ggsave("figures/Fig3.T0T1.vector.pdf", height=8, width=10)
...
inputstep2 =filter_data(inputtimepoint1only, 'Inoculum', filter_vals = '2')
inputstep3 =filter_data(inputstep2, 'Inoculum', filter_vals = '3')
...
shanfinalbindt0$inoculum<-as.factor(shanfinalbindt0$inoculum)
levels(shanfinalbindt0$inoculum) <- list(
  Dry="inoc1", Moderate="inoc4", Wet="inoc5", Control="inoc6")


shannonviolint0<-ggplot(shanfinalbindt0,aes(x=inoculum,y=shannon,color=factor(inoculum))) + 
...
shannonviolint0
ggsave("figures/FigS2.T0inoc.shannon.pdf", height=6, width=6)
...
shanfinalbindt1$inoculum<-as.factor(shanfinalbindt1$inoculum)
levels(shanfinalbindt1$inoculum) <- list(
  Dry="inoc1", Moderate="inoc4", Wet="inoc5", Control="inoc6")

shanfinalbindt1$type<-factor(shanfinalbindt1$type, levels=c("shoot", "root"))

shannonviolint1<-ggplot(shanfinalbindt1,aes(x=inoculum,y=shannon,color=factor(type))) + 
  ...
  scale_color_manual(values=c("mediumaquamarine", "sandybrown"))+
  theme(
    axis.text = element_text(color='black',size=10),
    axis.title = element_text(color='black',size=12),
    axis.ticks = element_line(color='black'),
...
shannonviolint1
ggsave("figures/FigS3.T1root.shoot.shannonv2.pdf", height=6, width=6)
...
# full model testing water and inoculum
fullmodel<-lm(shannon~type*water*inoculum,shanfinalbindt1)
Anova(fullmodel,type=3)
# type matters, and type:inolculum

posthoc.type<-emmeans(fullmodel,pairwise~type|inoculum)
#posthoc with letters
multcomp::cld(posthoc.type, Letters=letters)

posthoc.inoc<-emmeans(fullmodel,pairwise~inoculum|type)
#posthoc with letters
multcomp::cld(posthoc.inoc, Letters=letters)

######## separate by types
# separate root and shoot df
shan.shoot.T1<-shanfinalbindt1[(shanfinalbindt1$type=="shoot"),]
shan.root.T1<-shanfinalbindt1[(shanfinalbindt1$type=="root"),]

# Shoot model testing water and inoculum
shoot.mod<-lm(shannon~water*inoculum,shan.shoot.T1)
Anova(shoot.mod,type=3)
# no effects

# Root model testing water and inoculum
root.mod<-lm(shannon~water*inoculum,shan.root.T1)
Anova(root.mod,type=3)
# inoculum matters

posthoc.shan.root.T1<-emmeans(root.mod, pairwise~inoculum)
#posthoc with letters
multcomp::cld(posthoc.shan.root.T1, Letters=letters)
...
inputstep5 = filter_data(input, 'TimePoint', keep_vals = 'T1')
roott1 = filter_data(inputstep5, 'Type', keep_vals = 'Root')
roott1 = filter_data(roott1, 'Inoculum', filter_vals = '2')
roott1 = filter_data(roott1, 'Inoculum', filter_vals = '3')
shoott1 = filter_data(inputstep5, 'Type', keep_vals = 'Shoot')
shoott1 = filter_data(shoott1, 'Inoculum', filter_vals = '2')
shoott1 = filter_data(shoott1, 'Inoculum', filter_vals = '3')
time0 = filter_data(input,'TimePoint',keep_vals = 'T0')
shoott0 = filter_data(time0, 'Type', keep_vals = 'Shoot')
shoott0 = filter_data(shoott0, 'Inoculum', filter_vals = '2')
shoott0 = filter_data(shoott0, 'Inoculum', filter_vals = '3')
