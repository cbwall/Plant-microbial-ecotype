...
```{r above below biomass, width=20}
...
Stats for on the raw data for above and belowground plant biomass (not proportion).
Run models linear models (Type III SS) for *total root or shoot biomass*  
* First we will run the linear model as `lm(mass ~ Inoculation * Water)`
...
**Table. Model for inoculation and water effects on *total roots* biomass**
* no signifcant effects  
```{r root biomass}
...
Make a graph of the proportion of root and shoot biomass. 
* For shoots : `shoot biomass (mg) / total biomass (mg)`
* For roots : `root biomass (mg) / total biomass (mg)` 
```{r proportion graph p2, width=20}
...
input <- load_taxa_table("data/ASV_table_postfiltering.txt", "map_loaded_strawberries.txt")
```
...
timeinoculation<-ggplot(data=data.time,aes(x=NMDS1,y=NMDS2,color=factor(Inoculation),shape=factor(TimePoint))) + 
  ...
  geom_segment(aes(x = 0.27865686, xend = -0.35505773, y = -0.1568050, yend = -0.3410676),arrow = arrow(length = unit(0.25, "cm")), colour = "coral2") +
  ...
  geom_segment(aes(x = 0.38322169, xend = 0.13228889, y = -0.1386149, yend = 0.1352431),arrow = arrow(length = unit(0.25, "cm")), colour = "darkseagreen3") +
  theme(
    axis.text = element_text(color='black',size=10),
    axis.title = element_text(color='black',size=12),
    axis.ticks = element_line(color='black'),
